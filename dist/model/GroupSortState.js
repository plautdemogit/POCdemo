sap.ui.define([
	...
	return BaseObject.extend("namespace1.model.GroupSortState", {
		 ...
		 * @alias namespace1.model.GroupSortState
		 */
		...
		/**
		 * Groups by Ktwrt, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Ktwrt") {
				...
				this._oViewModel.setProperty("/sortBy", "Ktwrt");

				aSorters.push(
					new Sorter("Ktwrt", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Ebeln");
			}
